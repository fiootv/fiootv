...
export default function SMSForm({ customerId, customerPhone, conversations, onConversationAdded }: SMSFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (formData.message.length > 160) {
      toast.error('Message is too long. Please keep it under 160 characters.');
      return;
    }
                ...
                <div className="mt-1 relative flex-1 flex flex-col">
                  ...
                  <div className="absolute bottom-2 left-2 right-2 flex items-center justify-between">
                    <span className={`text-xs ${
                      formData.message.length > 160 ? 'text-red-400' : 'text-slate-400'
                    }`}>
          ...
          <CardContent className="p-0 h-full">
            {conversations.length === 0 ? (
              <div className="text-center py-12 h-full flex flex-col items-center justify-center">
                ...
                <p className="text-slate-500 text-sm mt-1">Start a conversation by sending an SMS</p>
              </div>
            ) : (
